...
;******************* Time Series on images *************************************

_rcvDetrendImage: routine [
    src 	[image!]
    dst  	[image!]
    /local
        pixS [int-ptr!]
    ...
    handle1: 0
    handleD: 0
    pixS: image/acquire-buffer src :handle1
...
_rcvNormalizeImage: routine [
    ...
    while [y < h] [
    	x: 0
       	while [x < w][ 
       		sum: sum + pixS/value	
       		sum2: sum2 + (pixS/value * pixS/value)	
        	pixS: pixS + 1
        	pixD: pixD + 1
        	x: x + 1
       ]
...
_rcvMMFilterImage: routine [
    ...
    /local
        pixS [int-ptr!]
        pixD [int-ptr!]
        handle1 handleD h w sz count n idx
        sum mm
][
    handle1: 0
    handleD: 0
    pixS: image/acquire-buffer src :handle1
    pixD: image/acquire-buffer dst :handleD
    sz: IMAGE_WIDTH(src/size) * IMAGE_HEIGHT(src/size)
    sum: 0
    count: 0
    while [count < (sz - filterSize)] [
    	n: 0
    	sum: 0
    	while [n < filterSize] [
    		idx: pixS + n
...
_rcvSGFilterImage: routine [
    ...
    /local
        ...
        pixSB
        handle1 handleD 
        kBase kValue kLength idx
        sg nr nl i n length sglength offset
		val sum coef sumCoef
][
    handle1: 0
    handleD: 0
    pixS: image/acquire-buffer src :handle1
    pixSB: image/acquire-buffer src :handle1
    pixD: image/acquire-buffer dst :handleD
    length: IMAGE_WIDTH(src/size) * IMAGE_HEIGHT(src/size)
    kBase: block/rs-head kernel
	kLength: (block/rs-length? kernel) - 2
	nl: kLength / 2
	nr: kLength / 2
	...
	while [i < (length - nr)] [
		...
		while [n < sglength][
    		...
    		kValue: KBase + n 
    		coef: as red-integer! kValue
    		sum: sum + (val * coef/value)
    		n: n + 1
    	]
    	kValue: kValue + 1
    	sumCoef: as red-integer! kValue
    	sg: sum / sumCoef/value
		pixD/value: sg
		pixS: pixS + 1
        pixD: pixD + 1
		i: i + 1
