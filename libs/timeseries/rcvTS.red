...
rcvDetrendImage: function [ src [image!] dst[image!]
"Removes linear trend in image by removing mean value of the image"
][
	_rcvDetrendImage src dst
]

rcvNormalizeImage: function [src [image!] dst[image!]
"Normalize image by replacing each pixel by a Z-normalized value"
][
	_rcvNormalizeImage src dst
]

rcvMMFilterImage: function [src [image!] dst[image!] filterSize [integer!]
"Calculates a mobile mean  according to the number of points given by filterSize"
][
	_rcvMMFilterImage src dst filterSize
]


rcvSGFilterImage: function [
"Calculates  polynomial Savitzky-Golay filter on image"
	src 		[image!]
	dst 		[image!]
	opSG		[integer!]
][
	;pre defined sg coefficients for fast calculation (quadratic and cubic polynomials)
	case [
		opSG = 1  [kernel: sgTable1/1] 	;5
    	opSG = 2  [kernel: sgTable1/2]	;7 
    	opSG = 3  [kernel: sgTable1/3]  ;9 
    ...
    _rcvSGFilterImage src dst kernel
]
