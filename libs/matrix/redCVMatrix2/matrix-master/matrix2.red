...
mx: context [
	...
	mtx: object [
		rows: cols: data: none
		get-col: func [col [integer!]][extract at to-block data col cols];because of %4409
		get-row: func [row [integer!]][copy/part at to-block data row - 1 * cols + 1 cols]
		remove-row: func [row [integer!]][remove/part at data get-idx row 1 cols rows: rows - 1 show]
		remove-col: func [col [integer!]][
			data: skip data col - 1
			loop rows [remove data data: skip data cols - 1]
			data: head data 
			cols: cols - 1 show
		]
		insert-row: func [block [block!] /at row [integer!] /local len [integer!]][
			row: any [row 1] 
		...
		append-row: func [block [block!]][insert-row/at block rows + 1]
		set-at: func [row [integer!] col [integer!] what [any-type!]][poke data row - 1 * cols + col what]
		get-idx: func [row [integer!] col [integer!]][index? at data row - 1 * cols + col]
		get-at: func [row [integer!] col [integer!]][pick data row - 1 * cols + col]
		get-row-idx: func [idx [integer!]][idx - 1 / cols + 1]
		get-col-idx: func [idx [integer!]][idx - 1 % cols + 1]
		get-diagonal: func [i [integer!] dir /local out [block!]][
			data: skip data i - 1 
		...
		to-float: has [d][d: to-block data forall d [d/1: system/words/to-float d/1] data: make vector! d]
		swap-dim: has [c [integer!]][c: cols cols: rows rows: c]
		...
		zero?: does [0 = sum data];mx-ctx/summa data]
		singular?: degenerate?: does [determinant = 0]
		invertible?: nonsingular?: nondegenerate?: does [not singular?]
		;summa: does [mx-ctx/summa self/data]
		product: does [mx-ctx/product self/data]
		...
		sub-exclude: func [rs [block! integer! none!] cs [block! integer! none!] /local m2 [object!]][ ; TBD
			m2: copy self
			switch type?/word rs [
				block! [
					sort/reverse rs
		...
		transpose: does [mx-ctx/transpose self]
		rotate: func [n [integer!]][mx-ctx/rotate n self]
		rotate-row: func [row [integer!] n [integer!]][mx-ctx/rotate-row row n self]
		rotate-col: func [col [block! integer!] n [block! integer!]][mx-ctx/rotate-col col n self]
		show: does [new-line/skip to-block data true cols]
		pretty: function [/tight /box][;/bar /local d i col-lengths][ m to
			col-lengths: copy []
			repeat i cols [
				c: to-block get-col i
			...
			step: either tight [sum col-lengths][(sum col-lengths) + cols - 1] ;mx-ctx/summa
		...
		swap-rows: func [r1 [integer!] r2 [integer!]][mx-ctx/swap-rows r1 r2 self]
		determinant: does [mx-ctx/determinant self]
		trace: does [mx-ctx/trace self]
		identity: func [/side d [word!]][either side [mx-ctx/identity/side self d][mx-ctx/identity self]]
		split-col: func [col [integer!]][mx-ctx/split-col col self]
		neighbours: func [row [integer!] col [integer!] /local out][
			out: copy []
			foreach [r c] case [
	...
	product: func [blk [block!] /local out][out: 1 forall blk [out: out * blk/1]]
	;summa: 	 func [blk /local out][out: 0 forall blk [out: out + blk/1]]
	determinant: func [m [object!] /local i r l rw minor mid idx][
		either m/square? [
	...
	add: func [op [op!] m1 [object!] m2 [object!] /local i][
	...
	multi: func [m1 [object!] m2 [object!] /local val i j k l][
	...
	kronecker: func [m1 [object!] m2 [object!] /local m3 i j k l][
	...
	rotate-row: func [r [integer!] n [integer!] m [object!] /local start-idx data][
		data: to-block m/data
		start-idx: m/get-idx r 1
		insert at data start-idx either negative? n [
			take/part at data start-idx - n m/cols + n 
		][
			take/part at data start-idx + m/cols - n n 
		]
		m/data: make vector! data
		m
	]
	rotate-col: func [c [block! integer!] n [block! integer!] m [object!] /local rows][
		either block? c [
			switch type?/word n [
	...
	augment: func [m1 [object!] m2 [object!] /local i j][
	...
	rref: func [n [object!] /local m i j c val][
		m: copy/deep n 
		m/to-float
		repeat i m/rows [
			; make the pivot
	...
	split-col: func [col [integer!] m [object!] /local data i j cls][
		data: copy []
		cls: m/cols - col + 1
		repeat i m/rows [
			j: m/rows - i + 1
			insert data to-block take/part at m/data m/get-idx j col cls 
		] 
		m/cols: col - 1
		reduce [m make mtx compose/only [rows: (m/rows) cols: (cls) data: make vector! (data)]]
	]
	invert: func [m [object!] /local n][
		augment m identity m
		n: rref m
		m: first split-col m/rows + 1 m
		second split-col n/rows + 1 n
	]
	game-of-life: func [m [object!] /local out c i][
		out: copy []
		c: m/data
		forall c [
	...
	set 'matrix func [
		spec [block!] 
		/local 
			dim dims rule result m w m1 m2 op op' var vars ops unary unaries d matrices res
	][
		...
		matrix-rule: [
			(m: none) [
				...
				(either (dim/1 * dim/2) = length? mdata: reduce mdata [
					m: make mtx [rows: dim/1 cols: dim/2 data: make vector! mdata]
				][
		...
		op-rule: [
			set op' ops-rule (insert ops op') expr-rule (
				...
				case [
					...
					true [case [
						find exclude ops-rule ['x 'X 'augment] op' [
		...
		if m1: take matrices [either number? m1 [m1][new-line/skip to-block m1/data true m1/cols]]
	]
]
