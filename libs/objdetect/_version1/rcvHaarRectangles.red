...
"Return the number of classes"
	...
	n: 			length? array
  	...
  	repeat i n [
  		root1: i
  		;find root1
  		while [gNodes/(root1)/1 >= 0] [root1: gNodes/(root1)/1]
  		j: 1 
      	repeat j n [
      		pred: rcvPredicate eps array/(i) array/(j)
      		if any [i == j not pred ][continue]
      		root2: j
      		while [gNodes/(root2)/1 >= 0] [root2: gNodes/(root2)/1]
      		if root2 <> root1 [ 
      			rank1: gNodes/(root1)/2
      			rank2: gNodes/(root2)/2
      			either rank1 > rank2 [gNodes/(root2)/1: root1][
      				gNodes/(root1)/1: root2
      				if rank1 == rank2 [
      					gNodes/(root2)/2: gNodes/(root2)/2 + rank1
      				]
      				root1: root2
      			]
  	...
  	repeat i n [
  		...
  		if gNodes/(root1)/2 >= 0 [
  			nclasses: nclasses + 1
  			gNodes/(root1)/2: complement nclasses
  		]
  		append labels complement gNodes/(root1)/2
  	]
...
"Group candidate by classes"
	...
	repeat i nclasses [
		...
		rrects/(i): r1				;--compute rectangle values as a functiontion od weight
	]
